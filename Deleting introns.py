...
introns = sequences[1:]
introns.sort(reverse=True)  # Adapting introns to DNA
for intron in introns:
    dna_string = dna_string.replace(intron, "")  # Deleting every intron
dna_string = dna_string.replace("T", "U")  # Making RNA from DNA
for i in range(0, len(dna_string) - 3, 3):
    print(rna_to_aminoacid_dictionary[dna_string[i: i + 3]], end='')   # RNA to protein